...
class AdminApp:
    def __init__(self, root):
        self.root = root
        self.root.title("進捗管理者パネル (Safe Distributed Edition)")
        self.root.geometry("850x900")

        # 1. 共有設定のパス確定と読込（既存接続 or 新規作成）
        if not self.init_config_management():
            self.root.destroy()
            return
    ...
    def init_config_management(self):
        """既存接続と新規作成を明確に分けて、不要な上書き警告を防ぐ"""
        self.config_path = get_local_config_path()
        
        # パスが未設定、または指し先（親フォルダ）が消えた場合
        if not self.config_path or not os.path.exists(os.path.dirname(self.config_path)):
            choice = messagebox.askyesnocancel(
                "初期設定", 
                "既存の config.json に接続しますか？\n\n"
                "【はい】既存のファイルを選択して接続\n"
                "【いいえ】新しい場所に config.json を作成"
            )
            
            if choice is True: # 既存接続
                self.config_path = filedialog.askopenfilename(
                    title="既存の共有config.jsonを選択",
                    filetypes=[("JSON files", "*.json")]
                )
            elif choice is False: # 新規作成
                self.config_path = filedialog.asksaveasfilename(
                    title="新しいconfig.jsonを作成",
        ...
        if os.path.exists(self.config_path):
            ...
            try:
                ...
                messagebox.showerror("生成エラー", f"ファイルを作成できませんでした:\n{e}")
                return False

    # (以下、setup_uiや各ロジックは前回と同じ)
    def setup_ui(self):
        ...
        ttk.Label(info_frame, text=f"共有設定: {self.config_path}", font=("Arial", 8), foreground="gray").pack(side="left")
        ttk.Button(info_frame, text="接続先変更", command=self.reset_connection).pack(side="right")
        ...
        ttk.Label(path_frame, text="ログ出力フォルダ:").pack(side="left")
        ...
        ttk.Button(action_frame, text="共有設定を保存(確定)", command=self.save_to_shared_config).pack(side="left", padx=10)
        ttk.Button(action_frame, text="全ログデータを集計出力", command=self.aggregate_data).pack(side="left", padx=10)

    def reset_connection(self):
        if messagebox.askyesno("確認", "設定の参照先を変更しますか？\n(アプリを再起動します)"):
    ...
    def sync_mapping_from_text(self):
        ...
        messagebox.showinfo("反映", "一時反映しました。最後に保存ボタンを押してください。")
    ...
    def save_to_shared_config(self):
        ...
        try: self.config["refresh_ms"] = int(self.refresh_entry.get())
        ...
        try:
            ...
            messagebox.showerror("保存失敗", f"保存できませんでした:\n{e}")
